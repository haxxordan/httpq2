...
BOOL CALLBACK AdvancedProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
        ...
        {
			// enable http headers
            if (ConfigDialog::Get()->GetConfig()->IsHttpHeadersEnabled())
                CheckDlgButton(hwndDlg, ID_ADVANCED_HEADERS_ENABLED, BST_CHECKED);
            else
			    CheckDlgButton(hwndDlg, ID_ADVANCED_HEADERS_ENABLED, BST_UNCHECKED);

			// connection timeout
            char timeout[256];
            wsprintf(timeout, "%d", ConfigDialog::Get()->GetConfig()->GetConnectionTimeout());
			SendDlgItemMessage(hwndDlg, ID_ADVANCED_TIMEOUT, WM_SETTEXT, 0, (LPARAM)(LPCTSTR)timeout);
        ...
        case WM_APP_SERIALIZE:
        {
            ...
            // http headers
			bool hdr = (IsDlgButtonChecked(hwndDlg, ID_ADVANCED_HEADERS_ENABLED) == BST_CHECKED);
            ConfigDialog::Get()->GetConfig()->SetHttpHeaders(hdr);

			// timeout value
			SendDlgItemMessage(hwndDlg, ID_ADVANCED_TIMEOUT, WM_GETTEXT, (WPARAM)sizeof(tmp), (LPARAM)&tmp);
			ConfigDialog::Get()->GetConfig()->SetConnectionTimeout(atoi(tmp));
...
BOOL CALLBACK SecurityProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
        ...
        {
			// ip allow/disallow 
            if (ConfigDialog::Get()->GetConfig()->IsAllowAllIpEnabled())
            {
                CheckDlgButton(hwndDlg, ID_SECURITY_ALLOW_ALL, 1);

                for(unsigned int i=0; i < ConfigDialog::Get()->GetConfig()->NumDeniedIp(); i++)
                {
                    DWORD dwAddr = ConfigDialog::Get()->GetConfig()->GetDeniedIp(i);
                    char *tmp = Server::Get()->ConvertToAddress(dwAddr, Server::ByteOrder::Host);
			        int cur = SendDlgItemMessage(hwndDlg, ID_SECURITY_ADDR_LIST, LB_ADDSTRING, 0, (DWORD)(LPSTR)tmp);
		            SendDlgItemMessage(hwndDlg, ID_SECURITY_ADDR_LIST, LB_SETITEMDATA, cur, (LPARAM)dwAddr);
                }
            }
            else
            {
			    CheckDlgButton(hwndDlg, ID_SECURITY_DENY_ALL, 1);

                for(unsigned int i=0; i < ConfigDialog::Get()->GetConfig()->NumAllowedIp(); i++)
                {
                    DWORD dwAddr = ConfigDialog::Get()->GetConfig()->GetAllowedIp(i);
                    char *tmp = Server::Get()->ConvertToAddress(dwAddr, Server::ByteOrder::Network);
			        int cur = SendDlgItemMessage(hwndDlg, ID_SECURITY_ADDR_LIST, LB_ADDSTRING, 0, (DWORD)(LPSTR)tmp);
		            SendDlgItemMessage(hwndDlg, ID_SECURITY_ADDR_LIST, LB_SETITEMDATA, cur, (LPARAM)dwAddr);
                }
            }
        ...
        case WM_APP_SERIALIZE:
        {
            ...
            ConfigDialog::Get()->GetConfig()->SetAllowAllIp(allow);
		...
		case WM_COMMAND:
        {
            switch(LOWORD(wParam))
            {
                case ID_SECURITY_ADD:
                {
                    ...
                    if (rc == IDOK)
                    {
                        DWORD dwAddr = ConfigDialog::Get()->GetLong();

                        if (ConfigDialog::Get()->GetConfig()->IsAllowAllIpEnabled())
                            if (ConfigDialog::Get()->GetConfig()->IsDeniedIp(dwAddr))
                                break;
                            else
                                ConfigDialog::Get()->GetConfig()->AddDeniedIp(dwAddr);
                        else
                            if (ConfigDialog::Get()->GetConfig()->IsAllowedIp(dwAddr))
                                break;
                            else
                                ConfigDialog::Get()->GetConfig()->AddAllowedIp(dwAddr);
                ...
                {
					int cur = SendDlgItemMessage(hwndDlg, ID_SECURITY_ADDR_LIST, LB_GETCURSEL, 0, 0);
					if(cur == -1 || cur == LB_ERR)
						break;
			
			        DWORD dwAddr = (DWORD)SendDlgItemMessage(hwndDlg, ID_SECURITY_ADDR_LIST, LB_GETITEMDATA, (int)cur, 0);

                    if (ConfigDialog::Get()->GetConfig()->IsAllowAllIpEnabled())
                        ConfigDialog::Get()->GetConfig()->RemoveDeniedIp(dwAddr);
                    else
                        ConfigDialog::Get()->GetConfig()->RemoveAllowedIp(dwAddr);

			        SendDlgItemMessage(hwndDlg, ID_SECURITY_ADDR_LIST, LB_DELETESTRING, (int)cur, 0);
				...
				{
			        SendDlgItemMessage(hwndDlg, ID_SECURITY_ADDR_LIST, LB_RESETCONTENT, 0, 0);
                    ConfigDialog::Get()->GetConfig()->EnableDenyAllIp();
				...
				{
			        SendDlgItemMessage(hwndDlg, ID_SECURITY_ADDR_LIST, LB_RESETCONTENT, 0, 0);
                    ConfigDialog::Get()->GetConfig()->EnableAllowAllIp();
